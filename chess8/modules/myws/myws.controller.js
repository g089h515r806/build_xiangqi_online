...
currentPlayer		
		...
		*/
		...
		switch(type) {
			...
			case "op_withdraw": opWithdrawCallback(ws, msgObj); break;
			...
			case "op_withdraw_response": opWithdrawResponseCallback(ws, msgObj); break;
...
function chatCallback(ws, msgObj){
	...
	//id不同于当前id clients , tableClients 里面存储的是ID，需要get一下
	tableClients.forEach(function each(clientId) {
	//clients.forEach(function each(client) {	
	  //if (client.readyState === WebSocket.OPEN && client.id !== ws.id) {
...
function opReadyCallback(ws, msgObj){
	...
	if(readyCnt < 2){
		...
		let redPlayer = players[0].name;
		let blackPlayer = players[1].name || "";
		players[0].isRed  = 1;
		msgObjRet	= {
...
function opResignCallback(ws, msgObj){
	...
	//gameTable.clients = clients;
	//gameTables.set(tableId, gameTable);
	//在原来消息上，增加一个opPlayer，发送给同游戏的用户，
	//当前操作用户收到这个消息，表示，服务器已经确认成功，就不用单独的_success状态了。
	/*
	msgObj.opPlayer = {
		id:ws.id,
		name:ws.name,
