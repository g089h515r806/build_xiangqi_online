...
currentPlayer		
		...
		*/
		...
		switch(type) {
			...
			case "reconnect": reconnectCallback(ws, msgObj); break;
			case "op_settings": opSettingsCallback(ws, msgObj); break;
			case "op_lookon": opLookonCallback(ws, msgObj); break;
			case "op_onlooker_leave": opOnlookerLeaveCallback(ws, msgObj); break;
...
// 重连回调函数
function reconnectCallback(ws, msgObj){
...
//旁观者离开回调
function opOnlookerLeaveCallback(ws, msgObj){
	let tableId = msgObj.tableId || "";
	let gameTable = gameTables.get(tableId);
    ...
    msgObj.gameTable = gameTable;	
	//console.log("opOnlookerLeaveCallback", msgObj);
	
	//先给当前用户发送一个通知，他已经从游戏中删除了，
    msgObj.user = {
		id:ws.id,
		name:ws.name,
	}	
...
//opQuitCallback 强退，后端逻辑
function opSettingsCallback(ws, msgObj){
	console.log("opSettingsCallback", msgObj);
	
	let tableId = msgObj.tableId || "";
	let gameTable = gameTables.get(tableId);
	let type = gameTable.type || "default";
	if(type !== "custom"){
		return;
	}
	...
	gameTable.settings = {
		time:settings.time || 600,
		stepTime:settings.stepTime || 60,
		addTime:settings.addTime,  // 0值的情况
	};
...
//opQuitCallback 强退，后端逻辑
function opLookonCallback(ws, msgObj){
	console.log("opSettingsCallback", msgObj);
