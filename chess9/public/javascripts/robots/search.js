...
// 对根节点的搜索
Search.prototype.searchRoot = function(depth, millis) {
//Search.prototype.searchRoot = function(depth) {	
  ...
  while ((mv = sort.next()) > 0) {
	//时间超时检查
    this.allMillis = new Date().getTime() - this.startMillis;	// 已经花费的时间
...
Search.prototype.searchMain = function(depth, millis) {
 ...
 for (var i = 1; i <= depth; i ++) {
   var vl = this.searchRoot(i, millis);
...
//console.log('mvResult', this.mvResult);
  return this.mvResult;
